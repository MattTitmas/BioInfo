...
def nwa(string_one: str, string_two: str, verbose: bool = False, overlap_detection: bool = False,
    ...
    for i in range(m+1):
        L[i][0] = (indel * i) if not overlap_detection else 0
        backtrace[i][0] = 'U'
    for j in range(n+1):
        L[0][j] = (indel * j) if not overlap_detection else 0
        backtrace[0][j] = 'L'
    backtrace[0][0] = ' '
...
def main(first_string: str, second_string: str, verbose: bool = False, overlap_detection = False):
...
if __name__ == '__main__':
    ...
    main(args.first, args.second, args.verbose, args.overlap_detection)
